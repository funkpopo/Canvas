...
export default function WorkloadsPage() {
  ...
  const daemonsets = workloads?.filter((w) => w.kind === "DaemonSet") ?? [];
    ...
    <div className="flex flex-col gap-6">
      ...
      <Tabs defaultValue="deployments" className="w-full">
        <TabsList className="grid w-full grid-cols-5">
          ...
          <TabsTrigger value="daemonsets">{t("workloads.tab.daemonsets")}</TabsTrigger>
        ...
        <TabsContent value="daemonsets" className="space-y-4">
          {isLoading ? (
            ...
            <Card>
              <CardContent className="flex items-center justify-center py-8">
                <div className="text-center space-y-2">
                  <p className="text-text-muted">{t("workloads.empty.daemonsets")}</p>
                  <p className="text-xs text-text-muted">{t("workloads.empty.daemonsets.hint")}</p>
                </div>
              </CardContent>
            </Card>
          ) : (
            <div className="grid gap-3 grid-cols-[repeat(auto-fill,300px)] justify-start">
              {daemonsets.map((workload) => (
                <Link
                  key={`${workload.namespace}-${workload.name}`}
                  href={`/workloads/daemonsets/${encodeURIComponent(workload.namespace)}/${encodeURIComponent(workload.name)}`}
                  className="block h-full"
                >
