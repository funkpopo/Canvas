...
import { Plus, Edit, Trash2, Bell, BellOff, Activity, AlertTriangle, Loader2 } from "lucide-react";
...
function AlertsPageContent() {
  ...
  const [ruleDialog, setRuleDialog] = useState<{ open: boolean; rule?: AlertRule }>({
    open: false,
  });
  ...
  const getRuleTypeLabel = (type: string) => {
    switch (type) {
      case "resource_usage":
        return t("ruleTypeResourceUsage");
      case "pod_restart":
        return t("ruleTypePodRestart");
      case "node_unavailable":
        return t("ruleTypeNodeUnavailable");
      default:
        return type;
    }
  };
                ...
                {events.map((event) => (
                  <div key={event.id} className="p-3 border rounded-lg">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">
                      {t("firstTriggeredAt", {
                        time: new Date(event.first_triggered_at).toLocaleString(),
                      })}
                    </div>
                  </div>
                ))}
