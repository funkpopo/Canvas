...
import { Search, Trash2, Loader2, Shield, Eye, KeyRound } from "lucide-react";
import {
  rbacApi,
  Role,
  RoleBinding,
...
export default function RBACPage() {
  ...
  const [clusterRoleBindingsNextToken, setClusterRoleBindingsNextToken] = useState<string | null>(
    null
  );
  ...
  }, [currentUser, selectedCluster, router, permissionErrorShown]);
  ...
  const fetchRoleBindings = async (options?: {
    append?: boolean;
  ...
  const fetchServiceAccounts = async (options?: {
    append?: boolean;
    continueToken?: string | null;
  }) => {
    if (!selectedCluster) return;
  ...
  const fetchClusterRoles = async (options?: {
    append?: boolean;
    continueToken?: string | null;
  }) => {
    if (!selectedCluster) return;
    ...
    try {
      ...
      if (response.data) {
        setClusterRoles((prev) =>
          append ? [...prev, ...response.data!.cluster_roles] : response.data!.cluster_roles
        );
  ...
  const fetchClusterRoleBindings = async (options?: {
    append?: boolean;
    continueToken?: string | null;
  }) => {
    if (!selectedCluster) return;
    ...
    try {
      ...
      if (response.data) {
        setClusterRoleBindings((prev) =>
          append
            ? [...prev, ...response.data!.cluster_role_bindings]
            : response.data!.cluster_role_bindings
        );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      switch (deleteDialog.type) {
        case "role":
          response = await rbacApi.deleteRole(
            selectedCluster.id,
            deleteDialog.namespace,
            deleteDialog.name
          );
          break;
        case "roleBinding":
          response = await rbacApi.deleteRoleBinding(
            selectedCluster.id,
            deleteDialog.namespace,
            deleteDialog.name
          );
          break;
        case "serviceAccount":
          response = await rbacApi.deleteServiceAccount(
            selectedCluster.id,
            deleteDialog.namespace,
            deleteDialog.name
          );
  ...
  const filteredRoleBindings = roleBindings.filter(
    (rb) =>
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="py-8">
            <div className="text-center text-muted-foreground">{t("selectClusterFirst")}</div>
          </CardContent>
        </Card>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title={t("title")}
        description={t("descriptionWithCluster", { cluster: selectedCluster.name })}
      />
        ...
        <TabsContent value="rolebindings">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("roleBindingsDescription")}</CardDescription>
              <div className="relative">
                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input
            ...
            <CardContent>
              {roleBindingsLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">{t("noRoleBindings")}</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
        ...
        <TabsContent value="serviceaccounts">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("serviceAccountsDescription")}</CardDescription>
              <div className="relative">
                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input
                ...
                <div className="flex justify-center pt-4">
                  <Button
                    ...
                    onClick={() =>
                      fetchServiceAccounts({
                        append: true,
                        continueToken: serviceAccountsNextToken,
                      })
                    }
        ...
        <TabsContent value="clusterroles">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("clusterRolesDescription")}</CardDescription>
              <div className="relative">
                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input
            ...
            <CardContent>
              {clusterRolesLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground">{t("noClusterRoles")}</div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
              ...
              {clusterRolesNextToken && (
                <div className="flex justify-center pt-4">
                  <Button
                    variant="outline"
                    onClick={() =>
                      fetchClusterRoles({ append: true, continueToken: clusterRolesNextToken })
                    }
        ...
        <TabsContent value="clusterrolebindings">
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("clusterRoleBindingsDescription")}</CardDescription>
              <div className="relative">
                <Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" />
                <Input
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open) =>
          !open && setDeleteDialog({ open: false, type: null, namespace: "", name: "" })
        }
