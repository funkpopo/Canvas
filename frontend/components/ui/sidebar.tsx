...
function SidebarTrigger({ className, onClick, ...props }: React.ComponentProps<typeof Button>) {
  ...
  return (
    <Button
      ...
      onClick={(event) => { toggleSidebar(); onClick?.(event) }}
      {...props}
    >
      <PanelLeft />
...
function SidebarInput({ className, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      data-sidebar="input"
      className={cn(
        "bg-background focus-visible:ring-sidebar-ring h-8 w-full rounded-md border px-2 text-sm shadow-none outline-none focus-visible:ring-2",
        className
      )}
      {...props}
    />
...
function SidebarSeparator({ className, ...props }: React.ComponentProps<"hr">) {
  return <hr data-sidebar="separator" className={cn("bg-sidebar-border mx-2 w-auto", className)} {...props} />
}
...
const sidebarMenuButtonVariants = (variant: "default" | "outline" = "default", size: "default" | "sm" | "lg" = "default") => {
  ...
  const variantClass = variant === "outline" ? "bg-background shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:bg-sidebar-accent hover:text-sidebar-accent-foreground hover:shadow-[0_0_0_1px_hsl(var(--sidebar-accent))]" : ""
  const sizeClass = size === "sm" ? "h-7 text-xs" : size === "lg" ? "h-12 text-sm group-data-[collapsible=icon]:!p-0" : "h-8"
...
function SidebarMenuSkeleton({
  ...
  return (
    <div data-sidebar="menu-skeleton" className={cn("flex h-8 items-center gap-2 rounded-md px-2", className)} {...props}>
      {showIcon && <div className="bg-sidebar-accent-foreground/10 size-4 rounded-md" />}
      <div className="bg-sidebar-accent-foreground/10 h-4 max-w-(--skeleton-width) flex-1 rounded-md" style={{ "--skeleton-width": width } as React.CSSProperties} />
    </div>
  )
}
...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "text-sidebar-foreground ring-sidebar-ring hover:bg-sidebar-accent hover:text-sidebar-accent-foreground active:bg-sidebar-accent active:text-sidebar-accent-foreground [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0 flex h-7 min-w-0 -translate-x-px items-center gap-2 overflow-hidden rounded-md px-2 outline-none focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50",
