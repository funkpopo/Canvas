...
def get_node_metrics(cluster: Cluster) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        for item in node_metrics.get('items', []):
            ...
            memory_percentage = (memory_usage / capacity['memory'] * 100) if capacity['memory'] > 0 else 0
...
def get_namespace_metrics(cluster: Cluster) -> Optional[List[Dict[str, Any]]]:
    ...
    try:
        ...
        for item in pod_metrics.get('items', []):
            ...
            for container in containers:
                ...
                if cpu_str.endswith('n'):
                    ...
                    namespace_usage[pod_namespace]['cpu'] += float(cpu_str)
        ...
        for ns in namespaces.items:
            ...
            usage = namespace_usage.get(ns_name, {'cpu': 0, 'memory': 0, 'pod_count': 0})
...
def install_metrics_server(cluster: Cluster, image: str = "registry.k8s.io/metrics-server/metrics-server:v0.7.0", insecure_tls: bool = False) -> bool:
